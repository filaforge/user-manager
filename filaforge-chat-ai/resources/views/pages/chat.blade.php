<x-filament-panels::page class="chat-ai-page">
    <x-filament::section>

        <div class="chat-ai-container">
		@if($viewMode === 'chat')
        <!-- Messages Area -->
        <div class="chat-ai-messages chat-ai-scroll" id="chat-ai-messages" x-ref="messages" x-data="{
            scrollToBottom() {
                this.$nextTick(() => {
            ...
            init() {
                ...
                this.$watch('messageSent', (val) => { if (val) this.scrollToBottom(); });
			...
			@if($showProfileApiKeyPrompt && $selectedProfileId)
				@php
			...
			@if(empty($messages))
				...
				<div class="chat-ai-overlay" aria-hidden="true" wire:target="send">
					<div wire:loading.remove wire:target="send" class="chat-ai-empty-state">
						<div class="flex flex-col items-center justify-center space-y-4">
							<x-filament::icon icon="heroicon-o-chat-bubble-left-right" class="chat-ai-empty-icon" />
					...
					<div wire:loading.class="is-loading" id="spinner-container" class="chat-ai-center-loader" aria-hidden="true" x-ref="spinner" wire:target="send">
						<div class="chat-ai-yellow-spinner"></div>
					</div>
				</div>
			@else
				@foreach($messages as $index => $message)
					@php $isUser = $message['role'] === 'user'; @endphp
					<div class="chat-ai-message {{ $isUser ? 'user' : 'ai' }}" x-data="{ entered: false }" x-init="setTimeout(() => entered = true, {{ $index * 100 }})" :class="{ 'entering': !entered, 'entered': entered }" wire:key="message-{{ $index }}">
						@if($isUser)
							<div class="chat-ai-message-bubble user"><div class="chat-ai-message-content user">{{ $message['content'] }}</div></div>
							<div class="chat-ai-message-avatar user">{{ substr(auth()->user()->name ?? 'U', 0, 1) }}</div>
						@else
							<div class="chat-ai-message-avatar ai">
								<x-filament::icon icon="heroicon-s-cpu-chip" class="chat-ai-ai-avatar-icon" />
							</div>
							<div class="chat-ai-message-bubble ai"><div class="chat-ai-message-content ai">{!! nl2br(e($message['content'])) !!}</div></div>
						@endif
					</div>
				@endforeach
			@endif

			<!-- Loading indicator handled via empty-state overlay above -->
		</div>
        ...
        <div class="p-4">
            <x-filament::section>
                ...
                <div wire:key="chat-ai-table-conversations">
                    {{ $this->table }}
                </div>
            </x-filament::section>
        </div>
			...
			<form x-on:submit.prevent="submit()" class="w-full">
				@if(!empty($availableProfiles))
					<div class="mb-2 flex items-center gap-2 text-xs">
						...
						<select wire:model.live="selectedProfileId" class="fi-input rounded-md border-gray-300 dark:bg-gray-800 dark:border-gray-700 dark:text-white text-xs">
							@foreach($availableProfiles as $p)
								<option value="{{ $p['id'] }}">{{ $p['name'] }} ({{ $p['model_id'] }})</option>
							@endforeach
						</select>
						<a href="{{ route('filament.admin.resources.chat-ai-models.index') }}" class="text-primary-600 dark:text-primary-400 hover:underline" target="_blank">manage</a>
					</div>
				@endif
				<div class="w-full">
					<x-filament::input.wrapper class="w-full chat-ai-textarea-outer" style="overflow: hidden;">
						<textarea class="fi-input chat-ai-textarea block w-full resize-none border-none bg-transparent text-base text-gray-950 placeholder:text-gray-500 focus:ring-0 focus:outline-none disabled:text-gray-500 disabled:cursor-not-allowed dark:text-white dark:placeholder:text-gray-400 sm:text-sm leading-relaxed" x-ref="input" x-model="draft" @keydown="handleKeydown($event)" placeholder="Type your message here... Enter to send, Ctrl + Enter for new line" rows="3" style="overflow: hidden; height: 3rem; min-height: 3rem; min-width: 100%; box-shadow: none !important; outline: none !important;"></textarea>
					</x-filament::input.wrapper>
				</div>
				<div class="chat-ai-send-row" style="padding: 0; background: transparent; border-radius: 12px; justify-content: flex-end;">
					<div class="flex items-center gap-3">
						<x-filament::button color="gray" icon="heroicon-o-plus" wire:click="newChatFromInput" id="chat-ai-new-chat-bottom" wire:key="chat-ai-new-chat-bottom">New Chat</x-filament::button>
